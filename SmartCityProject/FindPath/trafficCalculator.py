...
from FindPath.fileReader import fileReader
import FindPath.searchProblem as FPS
from FindPath.graphBuilder import graphBuilder
...
def processDay(data, day):
    ...
    # IDA* Segment
    t0 = time.time()
    availActions, costEstimation = FPS.idaStarCallback(sp)
    t1 = time.time()
    idatDiff = t1 - t0
    actualCost = sp.GetPathCost(availActions, sflag=True)
    idaCost = actualCost
